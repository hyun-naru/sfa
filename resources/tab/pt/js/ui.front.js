	...
	var ui = {
			...
			calendarOpen: function(target){
				...
				if(window.location.pathname.indexOf('/sb/pt/view.do') > -1){//청약화면이고
					if(sbComm && sbComm.prsSte == 3){//단계가 3단계 CDD화면
						maxFlag = 0;
						// $.datepicker._defaults.yearRange = '1900:c+0';
						$.datepicker._defaults.yearRange = 'c-100:c+0';
					}
				}

				if(iptDate.data('maxdate') != undefined) {
					maxFlag = iptDate.data('maxdate');
				}
				
				if(iptDate.data('mindate') != undefined) {
					minFlag = iptDate.data('mindate');
				}
			...
			swiperUpdate : function(){
				for( var i = 0 ; i < $('.swiperWrap.on:not(.off)').length ; ++i ){
					try{
						var element = window['ui' + $('.swiperWrap:eq('+i+')').attr('id') ];
						if(element){
							element.update();
						}
			...
			comboBoxInit : function(){
			    ...
			    $('body').off('click').on('click', closeAllComboBoxes);
